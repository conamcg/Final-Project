...
\*\*Structured output constraints:\*\* Each rubric call uses `max\_tokens=10` and instructs the model to respond only YES or NO. This eliminates hallucinated reasoning in the classification step and makes the output deterministic enough to parse reliably. Email drafting uses a strict format template with exact subject line prefixes and required closing lines.
...
`sample\_tickets\_7.csv` is included in this repo and contains 15 realistic synthetic tickets designed to produce approximately 25% escalation rate. Use this file to run a representative batch through the app.
...
5\. Upload `sample\_tickets\_7.csv` using the Batch CSV Upload tab to see a full triage run
...
!\[App Overview](screenshot\_overview.png)
...
!\[Escalation Detail](screenshot\_detail.png)
...
\- \[Anthropic Claude](https://www.anthropic.com/) — LLM classification and email drafting (claude-haiku-4-5)
