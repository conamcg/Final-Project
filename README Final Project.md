...
A set of 15 realistic synthetic tickets was designed to produce approximately 25% escalation rate (4 intended escalations). Tickets were crafted to test specific rubric questions — multi-site outages, patient impact language, recurring issues, VIP users — as well as clean DELEGATE cases with no ambiguous language.

**Results:** The model consistently identified the designed escalations. Over-escalation occurred on tickets with approval workflow language (Q4) and system behavior descriptions (Q2) that were intended as DELEGATE. After iterating on ticket language to remove ambiguity, the tool reached the target escalation rate. This revealed that Q2 and Q4 are the most sensitive criteria and most prone to false positives — a real finding with governance implications.
